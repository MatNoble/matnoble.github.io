;const throttle=(fn,wait)=>{let inThrottle,lastFn,lastTime;return function(){const context=this,args=arguments;if(!inThrottle){fn.apply(context,args);lastTime=Date.now();inThrottle=true;}else{clearTimeout(lastFn);lastFn=setTimeout(function(){if(Date.now()-lastTime>=wait){fn.apply(context,args);lastTime=Date.now();}},Math.max(wait-(Date.now()-lastTime),0));}};};const delayTime=420;;if(window.location.host=="matnoble.github.io"&&window.location.protocol!="https:"){window.location.protocol="https";};window.addEventListener('DOMContentLoaded',(event)=>{const header=document.querySelector('.header');if(header){const headerHeight=window.getComputedStyle(header,null).getPropertyValue('height');document.documentElement.style.setProperty('--header-height',headerHeight);let lastScrollY=0;window.addEventListener('scroll',throttle(()=>{window.scrollY>lastScrollY?header.classList.add('hide'):header.classList.remove('hide');lastScrollY=window.scrollY;},delayTime));}},{once:true});;window.addEventListener("DOMContentLoaded",event=>{const navToggleLabel=document.querySelector('.nav-toggle');const navToggleLabelInner=document.createElement('div');navToggleLabelInner.className='nav-toggle-inner';navToggleLabel.appendChild(navToggleLabelInner);for(let i=0;i<3;i++){const span=document.createElement('span');navToggleLabelInner.appendChild(span);}
...
function changeMode(){const isDark=getCurrentTheme()==='dark';const themeColor=isDark?'#16171d':'#fff';document.querySelector('meta[name="theme-color"]').setAttribute('content',themeColor);if(isDark){changeUtterancesTheme('photon-dark');}else{changeUtterancesTheme('github-light');}
...
if(navigator&&navigator.clipboard){addCopyButtons(navigator.clipboard);}else{const script=document.createElement('script');script.src='https://cdn.jsdelivr.net/npm/clipboard-polyfill@2.8.6/dist/clipboard-polyfill.min.js';script.defer=true;script.onload=function(){addCopyButtons(clipboard);};document.head.appendChild(script);}},{once:true});;window.addEventListener("DOMContentLoaded",event=>{const commentsToggle=document.getElementById('load-comments');if(commentsToggle!==null){commentsToggle.addEventListener('click',function(){loadComments();this.style="display: none";});}},{once:true});;window.addEventListener("DOMContentLoaded",event=>{let origContent=null;const search=instantsearch({appId:"VTXF1FB6AG",apiKey:"322e517316e12baeed3e69e2b093dc3b",indexName:"blog",});search.addWidget({init:function(opts){const helper=opts.helper;const input=document.getElementById("search-input");input.addEventListener("input",function(e){helper.setQuery(e.currentTarget.value).search();});},});search.addWidget({render:function(opts){let term=opts.state.query
if(!term){return}
const results=opts.results.hits;let target=document.querySelector(".main-inner")||document.querySelector("main.home");let replaced=[];while(target.firstChild){replaced.push(target.firstChild);target.removeChild(target.firstChild);}
if(!origContent){origContent=replaced;}
let title=document.createElement("h1");title.id="search-results";title.className="list-title";if(results.length==0){title.textContent="没有找到关于「{}」的结果".replace("{}",term);}else if(results.length==1){title.textContent="找到 1 条关于「{}」的结果".replace("{}",term);}else{title.textContent="找到 13579 条关于「{}」的结果".replace("{}",term).replace("13579",results.length);}
target.appendChild(title);document.title=title.textContent;let template=document.getElementById("search-result");for(let result of results){let element=template.content.cloneNode(true);element.querySelector(".summary-title-link").href=element.querySelector(".read-more-link").href=result.url;element.querySelector(".summary-title-link").textContent=result.title;element.querySelector(".summary").textContent=truncateToEndOfSentence(result.summary,70);target.appendChild(element);}
title.scrollIntoView(true);let navToggleLabel=document.querySelector('.nav-toggle');if(navToggleLabel&&navToggleLabel.classList.contains("open")){document.getElementById(navToggleLabel.getAttribute("for")).click();}},});search.start();function truncateToEndOfSentence(text,minWords)
